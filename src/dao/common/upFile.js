...
module.exports = {
    upFileForLocal: async (ctx) => {
        let write_stream = [], url = [], promise = [], file_array = [];
        ...
        file_array.map(item => {
            const origin_path = `/upload/${item.path.split('/').pop()}`; // 原始文件名路径
            // 用户自定义名字--->单文件上传
    ...
    upFileForOss: async ctx => {
        let local_array = [], promise = [], file_array = [], result_arr = [];
        const {type = null} = ctx.request.body;
        const files = ctx.request.files.file;	// 获取上传文件
        const isArray = Array.isArray(files);
        if (!isArray) {
